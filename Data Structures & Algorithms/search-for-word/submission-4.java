class Solution {
    public boolean exist(char[][] board, String word) {
        ...
        for (int i = 0; i < board.length; i ++) {
            for (int j = 0; j < board[0].length; j ++) {
                if (board[i][j] == word.charAt(0) && dfs(board, word, visited, i, j, curr)) return true;
            }
        }
        return false;
    }
    ...
    private boolean dfs(
        ...
        if (visited[i][j])
            return false;
        if (curr.length() > 1 && (curr.charAt(curr.length() - 1) != word.charAt(curr.length() - 1))) return false;
